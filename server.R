...
shinyServer(function(input, output) {
  ...
  observe({
    ...
    #queen
    wm_q <- poly2nb(subzones, queen=TRUE)
    ...
    localMI <- localmoran(aggregated_avg_priv_ec_by_subzone$X1, rswm_q)
    print(localMI)
    localMI[is.na(localMI)] <- 0
    print(localMI)
    
    # localMI.shade <- auto.shading(c(localMI[,1],-localMI[,1]),cols=brewer.pal(5,"PRGn"))
    # choropleth(subzones,localMI[,1],shading=localMI.shade)
    # choro.legend(170000, 2860000,localMI.shade,cex=0.7)
    # title("Hunan GDPPC 2012 (local Moran I)", cex.main=1.2)
    
    quadrant <- vector(mode="numeric",length=nrow(localMI))
    DV <- aggregated_avg_priv_ec_by_subzone$X1 - mean(aggregated_avg_priv_ec_by_subzone$X1)
    ...
    plot(subzones,border="lightgray",col=colors[findInterval(quadrant,brks,all.inside=FALSE)])
    box()
